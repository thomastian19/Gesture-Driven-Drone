...
I used a Cmod A7-35T (Xilinx Artix-7) board and Verilog to write the logic and verification. All timing was done using the 12MHz clock and counters, no MMCMs. 
...
### Output Generation
My original design received the UART packets and sent the data in the SBUS protocol. However, after finding the SBUS input on the radio required a solder (which I didn't have) and configuring that port wasn't well documented, I chose to change my design to send the data in PPM. The radio could receive this easily over a 3.5 mm aux port. 
...
by a gap, repeating continuously.
...
The interesting thing here was making the sync gap a variable duration, computed so each frame totals exactly 22.5 ms regardless of the channel values.
...
drone into a safe state rather than holding the last command. 

This was especially useful because my radio's trainer port was designed for 3.5 mm TS cable while I was using a 3.5 mm TRS cable. 
While the connection worked, it wasn't always stable and would lose connection occasionally. The failsafe prevented drone damage.


## ✅ Verification

**Staged Simulation** I wrote [testbenches](rtl/tb) covering the key scenarios and edge cases. I was able to exercise each
...
directly against the expected µs values.

**Saleae Logic 2 Software View**

<figure>
  <img width="596" height="188" alt="Screenshot 2026-06-25 200848" src="https://github.com/user-attachments/assets/ab557851-ee99-4ec2-9f98-e1ba3e19952b" />
...
are deliberately centered together, so nothing is commanded while you get into position.
...
One end of the 3.5 mm TRS cable went into the radio. I cut the other end, separated the wires, identified which wire went to which ring with
a multimeter in continuity mode, and hooked it up to the FPGA with a small breadboard.

The FPGA's output drives a RadioMaster Pocket through the cable to its trainer input, configured as a master trainer so the incoming channels drive the model.
A physical switch toggles the trainer on/off, providing a manual
